...
@section('content')
    <form class="uk-form-horizontal" action="{{url('groups/update')}}" method="POST">
        {{csrf_field()}}
                    ...
                    <div class="uk-grid uk-child-width-1-2" uk-grid>
                        @foreach($rights as $right)
                            <div class="uk-margin-small-bottom">
                                <label><input class="uk-checkbox" name="data[access-rights][]" value="{{$right->id}}" type="checkbox" {{$access->where('access_right_id', $right->id)->isNotEmpty() ? 'checked' : ''}}> {{$right->description}}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
